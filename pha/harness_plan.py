...
_FACT_CARD_INTERPRET_TASK = (
    ...
    "Window wording must match the card exactly. When assessment_compare is present, "
    "rolling-window phrases and compare stats must match that block (and Manifest); "
    "do not invent another N; do not relabel progressive-baseline numbers as the "
    "assessment window. Progressive baseline (e.g. 90d) may still be cited when on "
    "the card. If assessment_compare is absent, do not write 近/过去/最近 N 天|日|周 "
    "except window phrases already on the card; never invent 近 14/15 日 to frame a "
    "personal mean. When assessment_point_compare (yesterday) is present, cite only those "
    "yesterday values/deltas from the card. Do not invent or derive extra "
