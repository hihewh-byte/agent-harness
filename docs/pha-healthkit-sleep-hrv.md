...
> **Language / 语言**：[English](pha-healthkit-sleep-hrv.en.md) · 中文（本文）

> 状态：`DONE` · 2026-09-10 T9 维护者口头确认 9/8–9/10 健康 App vs 日表「基本都对」（入睡/清醒/核心/深睡/REM；在床不验收）。HRV 真机已对上。T0–T8 落地。锚点 A 过门禁。  
...
**当前实现偏差（2026-09-10）**：无。T0–T9 均已落地。T9 = 维护者口头确认 9/8–9/10 基本对上（不含在床）。

| 项 | 规范 | 现状 |
|---|---|---|
| 醒来日 / 正午窗口 / 总并集 / 分段+审计 | 见上文 | **已落地**（T1–T2） |
| 清醒 + 偏离基线提醒 | 见上文 | **已落地**（T3）；9/10 深睡 below → 核对句、无判定词 |
| 在床只取 In Bed；无则空 | 见上文 | **已落地**（T4）；日表 `in_bed` 全空。2026-09-10：维护者已从健康 App 删除在床，T9 不验收；孤儿 `sleep_in_bed` 日键 2 条已删 |
| 同日 sleep_* 日键清理 | 见上文 | **已落地**（T5）；无 `healthkit|…|sleep_*` 孤儿行 |
| 捷径窗口 | 覆盖昨夜；有则取 In Bed | D1：last 2 days + Limit 150；Source/Device 空（Shortcuts 对 Sleep 不吐来源） |
| 真源 = 健康 App 展现 | 按系统优先级只留展示源 | **M1 过渡**：维护者删 Pillow 后单源≈展现；T9 9/8–9/10 口头确认通过；多源自动对齐属 M2 |
...
**T9 证据（2026-09-10）**：维护者在健康 App 核对醒来日 9/8、9/9、9/10，口头确认与日表「基本都对」，算通过。比对项：入睡 / 清醒 / 核心 / 深睡 / REM。在床不验收。库内对应：
...
1. 同一夜：健康 App 醒来日 D vs 日表。**只验收健康 App 当天有、且用户已勾选的项**，各 ≤ ±10 分钟；健康 App 没有的项 PHA 必须空，双方皆空算过。分期互斥时写分期列；`stage_overlap` 时分期列空、只保留入睡总并集。第一夜证据 = 锚点 A 的 15:41 行。**T9 三夜不比对在床**（维护者 2026-09-10：健康 App 已删除 In Bed）。  
...
按 review §7 任务表 **T0 → T9** 已全部落地。T9 = 2026-09-10 维护者口头确认。M1-P6 = `DONE`。
