...
| FR-2.8 | **通用参考层（Tier 1 披露制）**：个人基线之外，对 **有人群参考意义** 的指标给一行参考范围内/外，格式沿用 `manifest-tier-v1` 的 T1 披露块：`【参考标准】<描述>（来源：<指南/组织>，请自行查证，非医疗建议）`。参考范围是 **数据**，放注册表 `fact_card.reference_range`（低/高/单位/来源），Python 不写死数字。v1 适用：睡眠总时长、静息心率、步数；**不适用**：HRV 绝对值（个体差异过大，只跟自己比）、清醒、在床、**深睡/REM 占比**（用户可由分期小时自行计算；无 sourced 指南前禁止编造人群范围） | 完整卡每个适用指标多一行参考句；数字 ⊆ 注册表；`fact_card_numeric_atoms` 把参考范围也纳入原子集；非医疗措辞；HRV / 深睡占比 / REM 占比无参考句 |
...
| FR-6.8 | **证据源**：解读的唯一一等证据源是当日事实卡 JSON（facts + 递进基线 + 参考层）；以 `fact_card_interpret` profile 进入 harness，Tier0 只含 TASK / NUMERICS_MANIFEST（由卡生成）/ FACT_CARD_CONTEXT / USER_ASSESSMENT_PROMPT；`WEARABLE_90D_SUMMARY` 等固定窗口摘要禁止进入该路径。解读中任何窗口、n、日期必须与卡上规则层逐字一致。**解读轮不套三步看诊法**（`PHA_FACT_CARD_SOUL_MINIMAL`，见 M1-P9.5）。**唯一 Tier1** 为非数字源 `USER_BACKGROUND_BRIEF`（FR-6.12），不得把 brief 内数字并入 manifest。**大纲分档（v1.14）**：整卡仍是证据源；叙述是否覆盖未点名行由 Intent Catalog `assessment_outline.outline_mode` ∈ `{exclusive, emphasis, cover-card}` 决定（exclusive > emphasis > cover-card）。`exclusive`（只看/仅/only）维持 P9.5b：只谈点名行、未点名不得另起段。`emphasis`（重点看/侧重/especially）：点名为主段，其余勾选有值行允许同一段带过、不得另起专题段。**结构句（v1.15）**：emphasis 的英文 TASK 第 1 句 = overall readiness（句中无 metric label）；随后才按评估要求点名。`exclusive` **不得**要求整体起笔。`cover-card`：无指标 token 或纯 holistic/daily_readiness 且未命中前两档 → 覆盖勾选有值行。`daily_readiness` 不得默认 exclusive。Python **不**解析评估要求里的指标 id。不得把 emphasis 改成「有值必须点到」（那是 cover-card）；不得缺行再跑一轮 LLM。**exclusive 注入两层（v1.16）**：用户可见 HTML 卡仍整卡勾选；仅 exclusive 解读轮的 `FACT_CARD_CONTEXT` + Numerics Manifest ⊆ 本轮 catalog `infer_wearable_metric_ids`（现有函数，不新写评估要求解析器）。emphasis / cover-card 仍整卡注入。注入为空 fail-closed，不调 LLM。Flag `PHA_EXCLUSIVE_INJECT_NAMED`（默认 1，关则 exclusive 回到整卡注入）。**起笔/训练（v1.16）**：整体与训练建议以规则层（卡上 summary/advice）为准；P19 Sentence 1 保留不加字；评测金标 `lead_not_overall` / `training_unanswered` 退出产品 P0，只留 telemetry。禁止 Python 按 band 生成「可以力量训练」。TASK 通则：有值不得说缺失；`partial_day` 不是缺失；不复述卡头同步免责。**TASK 槽契约（v1.19）**：禁止从卡上百分位再派生新百分比；人群整数不得贴到卡指标标签上（除非 T1 块）。`USER_BACKGROUND_BRIEF` 在场则 in-scope，禁止整槽 skip；只塑注意事项措辞，不引为数值、不复述剂量、不强制另起编号注意事项段。禁止药名表。数字真源仍是 Manifest / FACT_CARD_CONTEXT，**不是** CHB §Facts | selfcheck 注入「近 90 天」于 365d 卡 → 拒；O1 exclusive 不谈未点名血氧；O2 emphasis = daily_readiness。P19-T 结构句；P20 exclusive 注入 ⊆ 点名行。交接 [`handoff-2026-09-10-eval-audit-solution.md`](handoff-2026-09-10-eval-audit-solution.md) |
...
| **M1-P1** | iPhone 捷径本地通知（可配每日自动化） | `DONE` | 2026-09-06：LAN `<LAN-IP>` `GET /proactive/fact-card` **200**；维护者确认 iPhone 弹出事实卡通知。每日自动化仍由「快捷指令」定时跑，未另验。 |
...
| **M1-P6** | 睡眠各项 + HRV SDNN | `DONE` | 2026-09-10：T9 维护者口头确认 9/8–9/10 健康 App vs 日表「基本都对」（入睡/清醒/核心/深睡/REM；在床已从健康 App 删除，不验收）。HRV 真机已对上。T0–T8 落地。锚点 A 15:41 ≤8 分钟。最近睡眠 ingest 200（40 段，`stage_overlap=false`）。`pha_healthkit_ingest_selfcheck` PASS。见任务卡 / review |
| **M1-P7** | **递进个人基线 + 通用参考层**（无 LLM） | `DONE` | 2026-09-07：按指标 90d→365d→all；真机睡眠 `365d` n=267；注册表 `reference_range`（睡眠总时长/RHR/步数）+ T1 披露；卡级综合/不综合；selfcheck PASS。2026-09-10：维护者定深睡/REM 占比不另做人群参考（用户可自行计算；无 sourced 指南不编造） |
...
| **M1-P12** | **优先级包一期**：闭合 zip 日表（血氧 / 呼吸率 / VO2max / 腕温）+ zip 为最终真值 + pack_version | `DONE` | 2026-09-08：7 项数量 Find 真机入库；Find 总表 `shortcut_health_find_catalog.json`。VO2max `latest` 不计覆盖率；zip 覆盖同日 healthkit。pack `2026.09.08.priority-1e`。**腕温**：捷径仍 `shortcuts_find_unverified`（无权限开关）。2026-09-10 维护者定：腕温权限推迟到第二批指标进入勾选名单时一并处理，本卡不再用 `*` 挂起 |
...
| **M1-P20** | **exclusive 注入 ⊆ catalog 点名行**（FR-6.8 v1.16） | `DONE*` | 2026-09-10：仅 exclusive 解读轮裁注入。离线 `pha_p20_selfcheck`。Mac 真卡「只看今天的深睡」注入仅深睡、审计过。iPhone 同网待验 |
| **M1-P15** | **CHB 统一供给**（C 层 · 闭环） | `IN_PROGRESS` | v1.20：脉络丢指标字段残行。黄金句注意事项未过前不标 DONE |
| **M1** | （汇总）iPhone 主动事实卡：通道 + 完整卡 + 可选指标 | `DONE*` | P0–P7 / P12 已落地。`*` = P15 验收中、P20 iPhone 同网待验。未开 M2。腕温进第二批勾选。 |
...
| 2026-09-10 | 40+40 评测：emphasis 从 RHR 起笔且 selected_gap 金标用错；Tier0 Manifest 尾截断；黄金句对话 workout 簇误入 scope。维护者授权按审计改写开工，**驳回** must 覆盖与 prefs∩ | v1.15：FR-6.8 结构句；FR-6.13 发卡 ⊆ FACT_CARD_CONTEXT；立 M1-P19。交接 [`handoff-2026-09-10-eval-audit-solution.md`](handoff-2026-09-10-eval-audit-solution.md) |
| 2026-09-10 | 维护者口令按 1–5 开工（跳过换模型）：brief 读侧丢问句；exclusive 注入两层；起笔/训练退出产品 P0；开 P15 CHB | v1.16：FR-6.8 exclusive 注入 + 规则层为准；FR-6.12 读侧 negative + CHB 供给；立 M1-P20；M1-P15 DONE* |
| 2026-09-10 | M1-P6 T8 文档收口：T7 回执已落地，偏差表无剩余实现差。库内 9/8–9/10 三夜满足跨度/并集内部门禁。T9 仍须健康 App 截图对账，**不**标 DONE | §8 P6 仍 IN_PROGRESS；任务卡/review 对齐 |
| 2026-09-10 | 维护者确认：健康 App 已删除在床（In Bed）样本，T9 不再验收在床；PHA 不用再管该列。查库日表 `in_bed_hours` 已全空；残留 2 条非 `healthkit\|` 孤儿 `sleep_in_bed` 日键（9/6 半截 POST 0.73h、9/7 Pillow 8.93h）已删。ingest 仍保留「有 In Bed 样本才写、没有就空」，不从白名单拆掉指标 | 备份 `data/backups/pha_storage.20260910T082534Z.db`；§8 P6 / 任务卡 T9 排除在床 |
| 2026-09-10 | 维护者口头确认 T9：9/8–9/10 健康 App vs PHA 日表「基本都对」，可以算通过 | §8 M1-P6 → `DONE`；任务卡 / review / change-log 收口。未开 M2 |
...
| 2026-09-10 | v1.15 | FR-6.8 emphasis 第 1 句 overall；FR-6.13 `wearable_daily_review` 发卡 ⊆ FACT_CARD_CONTEXT；Tier0 禁止尾切 Manifest/卡上行 KV；workout hints 与 daily_readiness 训练词解绑。立 M1-P19。交接 [`handoff-2026-09-10-eval-audit-solution.md`](handoff-2026-09-10-eval-audit-solution.md) |
| 2026-09-10 | v1.16 | FR-6.8 exclusive 注入 ⊆ catalog 点名行（可见卡仍整卡）；起笔/训练以规则层为准，`lead_not_overall`/`training_unanswered` 退出产品 P0；FR-6.12 读侧与捕获同一 negative 词表；M1-P20 / M1-P15 编码。换模型未做 |
| 2026-09-10 | v1.16 | §8 M1-P6 → DONE：T9 维护者口头确认 9/8–9/10 基本对上（不含在床） |
